...
export default class CollisionManager {
    ...
    handleBulletEnemyCollision(bullet, enemy) {
        ...
        this.scene.effects.showDamage(enemy.x, enemy.y, damage);

        const isDead = enemy.takeDamage(damage);

        if (!isDead) {
            this.scene.soundManager?.sfx("hit", 0.25);
        ...
        const isElite = enemy.stats?.elite === true;

        enemy.destroy();

        this.scene.waveManager.enemyKilled();
        this.scene.addScore(enemy.stats.score);
        ...
        this.scene.soundManager?.sfx("explosion", isElite ? 0.65 : 0.45);
        this.scene.effects.createExplosion(explosionX, explosionY);

        if (isElite) {
            this.scene.cameras.main.flash(180, 255, 200, 0);
            this.scene.cameras.main.shake(260, 0.01);

            for (let i = 0; i < 5; i++) {
                this.scene.time.delayedCall(i * 70, () => {
                    this.scene.effects.createExplosion(
                        explosionX + Phaser.Math.Between(-45, 45),
                        explosionY + Phaser.Math.Between(-45, 45)
                    );
            ...
            if (Math.random() < 0.5) {
                this.scene.powerUpManager.spawnReward(
                    explosionX,
                    explosionY,
                    "shield"
                );
            ...
            this.scene.powerUpManager.tryDropPowerUp(
                explosionX,
                explosionY
            );
        }
    ...
    checkBulletHitsMine() {
        ...
        this.scene.bullets.getChildren().forEach((bullet) => {
            ...
            this.scene.mines.getChildren().forEach((mine) => {
                if (!mine?.active || !bullet?.active) return;
    ...
    checkMineHitsPlayer() {
        if (this.scene.isGameOver || this.scene.playerInvincible) return;
        if (!this.scene.player?.active || !this.scene.mines) return;

        this.scene.mines.getChildren().forEach((mine) => {
            if (!mine?.active || !mine.isArmed?.()) return;
            ...
            if (distance < mine.triggerRadius) {
                this.explodeMine(mine, false);

                if (this.scene.powerUpManager?.hasShield?.()) {
                    this.scene.powerUpManager.breakShield();
                    return;
                }

                this.scene.soundManager?.sfx("hit", 0.55);
                this.scene.damagePlayer();
            }
        });
    }

    explodeMine(mine, giveScore = true) {
        if (!mine?.active) return;

        const x = mine.x;
        const y = mine.y;
        const score = mine.scoreValue || 60;

        mine.destroy();

        this.scene.soundManager?.sfx("explosion", 0.5);
        this.scene.effects.createExplosion(x, y);
        this.createMineShockwave(x, y);

        this.scene.cameras.main.shake(90, 0.006);
        this.scene.cameras.main.flash(120, 255, 120, 40);

        if (giveScore) {
            this.scene.addScore(score);
            this.scene.effects.showDamage(x, y, score);
        }
    }
    ...
    createMineShockwave(x, y) {
        const ring = this.scene.add.circle(
            ...
            0xff7a18,
            0.18
        ).setStrokeStyle(4, 0xfb923c, 1).setDepth(40);
    ...
    checkBulletHitsAsteroid() {
        ...
        this.scene.bullets.getChildren().forEach((bullet) => {
            ...
            this.scene.asteroids.getChildren().forEach((asteroid) => {
                if (!asteroid?.active || !bullet?.active) return;
                ...
                const hitRadius = asteroid.damageRadius || 48;
                ...
                if (distance < hitRadius) {
                    ...
                    if (!isDead) {
                        this.scene.soundManager?.sfx("hit", 0.25);
                        this.flashTarget(asteroid);
                        this.scene.cameras.main.shake(25, 0.0015);
                        return;
                    }
    ...
    checkAsteroidHitsPlayer() {
        if (this.scene.isGameOver || this.scene.playerInvincible) return;
        if (!this.scene.player?.active || !this.scene.asteroids) return;

        this.scene.asteroids.getChildren().forEach((asteroid) => {
            if (!asteroid?.active || !this.scene.player?.active) return;
    ...
    destroyAsteroid(asteroid, giveScore = true) {
        if (!asteroid?.active) return;

        const x = asteroid.x;
        const y = asteroid.y;
        const score = asteroid.scoreValue || 75;

        asteroid.destroy();

        this.scene.soundManager?.sfx("explosion", 0.4);
        this.scene.effects.createExplosion(x, y);
    ...
    checkEnemyHitsPlayer() {
        ...
        this.scene.enemies.getChildren().forEach((enemy) => {
            ...
            if (distance < 55) {
                ...
                this.scene.soundManager?.sfx("explosion", 0.55);
                this.scene.effects.createExplosion(explosionX, explosionY);
